...
/**
 * How long it stands there when it gets there, in seconds.
 *
...
export function startWander(
  ...
  return { home: { ...home }, goal: spotNear(home, range, random), pause: 0 };
}
...
export function steerWander(
  ...
  // thirty metres above.
  if (!isPerched(state)) return controls;
  ...
  const dx = wander.goal.x - state.position.x;
  const dz = wander.goal.z - state.position.z;
  if (Math.hypot(dx, dz) <= ARRIVED) {
    wander.pause = REST_LEAST + random() * (REST_MOST - REST_LEAST);
    return controls;
  }
  ...
  controls.forward = Math.abs(off) < WALK_WITHIN ? 1 : 0;
  return controls;
}
